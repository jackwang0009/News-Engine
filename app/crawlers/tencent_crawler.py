...
class TencentCrawler(WebsiteCrawler):
    """腾讯新闻爬虫"""
    ...
    def __init__(self, source_id: str, source_url: str, **kwargs):
        ...
        self.article_selectors = {
            'news_list': '.list .item, .news-item, .feed-item',
            'title': 'h1, h2, h3, .title, .headline, .item-title',
            'content': '.summary, .desc, .content, .item-desc',
            'author': '.author, .reporter, .writer, .item-author',
            'publish_time': '.time, .date, .item-time',
            'category': '.category, .channel, .item-category'
        }
    ...
    def extract_articles(self, soup: BeautifulSoup, page_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not news_items:
                # 尝试其他选择器
                news_items = soup.select('article, .news, .post, .item')
